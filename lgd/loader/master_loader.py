...
from torch_geometric.datasets import (Amazon, Coauthor, GNNBenchmarkDataset, TUDataset, Planetoid,
...
from lgd.transform.expander_edges import generate_random_expander
from lgd.transform.dist_transforms import (add_dist_features, add_reverse_edges,
                                                 add_self_loops, effective_resistances,
                                                 effective_resistance_embedding,
                                                 effective_resistances_from_embedding)
from lgd.transform.transforms import (add_virtual_node_edge, no_virtual_node_edge, pretransform_QM9_generation,\
                                        pretransform_pe, preprocess_edge_inductive)
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        #     dataset = Planetoid(dataset_dir, name)
            ...
            if cfg.dataset.get('split_mode', 'random') == 'standard':
                dataset = Planetoid(dataset_dir, name, split='public')
            else:
                from .planetoid import Planetoid as Planetoid_Custom
                dataset = Planetoid_Custom(dataset_dir, name, split='random', train_percent=cfg.prep.train_percent)
            ...
            if cfg.prep.add_self_loops == True:
                pre_transform_in_memory(dataset, partial(add_self_loops))
            pre_transform_in_memory(dataset, partial(preprocess_edge_inductive))

        elif pyg_dataset_id == 'Spectre':
            dataset = preformat_Spectre(dataset_dir, name)
        ...
        elif pyg_dataset_id == 'MOSES':
            dataset = preformat_MOSES(dataset_dir, name)

        else:
            raise ValueError(f"Unexpected PyG Dataset identifier: {format}")

    # GraphGym default loader for Pytorch Geometric loader
    elif format == 'PyG':
        processed_path = dataset_dir + '/' + name + '/processed.pt'
        if os.path.exists(processed_path):
            dataset = torch.load(processed_path, map_location="cpu")
        ...
        ### Link prediction loader.
        elif name.startswith('ogbl-'):
            # GraphGym default loader.
            dataset = load_ogb(name, dataset_dir)
            # OGB link prediction loader are binary classification tasks,
    ...
    if cfg.dataset.add_virtual_node_edge:
        logging.info('Adding virtual node and edges')
        pre_transform_in_memory(dataset,
                                partial(add_virtual_node_edge, format=cfg.dataset.format),
                                show_progress=True
                                )
    elif cfg.dataset.get('aug_edge', True):
        logging.info('Augmenting edges')
        pre_transform_in_memory(dataset,
                                partial(no_virtual_node_edge, format=cfg.dataset.format),
                                show_progress=True
                                )

    if cfg.dataset.get('aug_edge', True) or cfg.dataset.add_virtual_node_edge:
        logging.info('Preprocessing PE')
        pre_transform_in_memory(dataset,
                                partial(pretransform_pe, add_virtual_node=cfg.dataset.add_virtual_node_edge),
                                show_progress=True
                                )

    if cfg.dataset.subgraph:
        pre_transform_in_memory(dataset,
                                partial(T.RootedEgoNets, hop=cfg.dataset.num_hop),
                                show_progress=True
                                )
...
def preformat_GNNBenchmarkDataset(dataset_dir, name):
    """Load and preformat loader from PyG's GNNBenchmarkDataset.
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from lgd.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_OGB_PCQM4Mv2(dataset_dir, name):
    ...
    if name == 'full':
        valid_half_len = len(split_idx['valid']) // 2
        split_idxs = [train_idx,  # Subset of original 'train'.
                      split_idx['valid'][:valid_half_len],
                      # valid_idx,  # Subset of original 'train' as validation set.
                      split_idx['valid'][valid_half_len:]  # The original 'valid' as testing set.
                      ]
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from lgd.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from lgd.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from lgd.loader.dataset.peptides_structural import \
...
def preformat_TUDataset(dataset_dir, name):
    """Load and preformat loader from PyG's TUDataset.
    ...
    if name in ['DD', 'NCI1', 'ENZYMES', 'PROTEINS']:
        func = None
    elif name.startswith('IMDB-') or name == "COLLAB" or name in ['MUTAG', 'PTC']:
        func = T.Constant()
    else:
...
def preformat_ogbn(dataset_dir, name):
  if name == 'ogbn-arxiv' or name == 'ogbn-proteins':
    ...
    for split in ['train', 'val', 'test']:
        mask = torch.zeros([dataset.data.x.shape[0]])
        mask[split_dict[split]] = 1
        dataset.data[split+'_mask'] = mask.bool()
    return dataset
...
def preformat_ZINC(dataset_dir, name):
    """Load and preformat ZINC loader.
    ...
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[171].x)
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[172].x)
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[173].x)
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[266].x)
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[267].x)
    # logging.info(ZINC(root=dataset_dir, subset=(name == 'subset'), split='val')[268].x)
    return dataset


def preformat_Spectre(dataset_dir, name):
    from lgd.loader.dataset.synthetic_dataset import SpectreGraphDataset
    dataset = join_dataset_splits([SpectreGraphDataset(dataset_name=name, root=dataset_dir, split='train'),
                                   SpectreGraphDataset(dataset_name=name, root=dataset_dir, split='val'),
                                   SpectreGraphDataset(dataset_name=name, root=dataset_dir, split='test')])
    return dataset


def preformat_MOSES(dataset_dir, name):
    from lgd.loader.dataset.moses import MOSESDataset
    dataset = join_dataset_splits([MOSESDataset(stage='train', root=dataset_dir, filter_dataset=True),
                                   MOSESDataset(stage='val', root=dataset_dir, filter_dataset=True),
                                   MOSESDataset(stage='test', root=dataset_dir, filter_dataset=True)])
    logging.info('MOSES_bond: min = ' + str(torch.min(dataset.data.edge_attr)) + ' , max = ' + str(
        torch.max(dataset.data.edge_attr)))

    return dataset


def preformat_QM9(dataset_dir, name):
    """Load and preformat QM9 loader.
    ...
    """
    if name == 'standard':
        QM9_dataset = QM9(root=dataset_dir)
        excluded_strings = []
        gdb9_txt_excluded = osp.join(QM9_dataset.raw_dir, 'uncharacterized.txt')
        ...
        # Ntrain = 100000
        Ntrain = Nmols - 5000
        if cfg.dataset.get("half", False):
            Ntrain = 50000
        Ntest = 2500
        Nvalid = 2500
        # Ntest = int(0.1 * Nmols)
        # Nvalid = Nmols - (100000 + Ntest)
        ...
        if not cfg.dataset.get("half", False):
        ...
        dataset = join_dataset_splits(
            [QM9_dataset[train], QM9_dataset[valid], QM9_dataset[test]]
        )

        mean = QM9_dataset[train].data.y.mean(dim=0)
        std = QM9_dataset[train].data.y.std(dim=0)

        pre_transform_in_memory(dataset,
                                partial(pretransform_QM9_generation, target=cfg.dataset.get("target", None), align=True, add_virtual_node=cfg.dataset.get("add_virtual_node_edge", False),
                                        mean=mean, std=std),
                                show_progress=True)

    else:
        from lgd.loader.dataset.qm9 import QM9Dataset
        dataset = join_dataset_splits([QM9Dataset(stage='train', root=dataset_dir, remove_h=cfg.dataset.remove_h),
                                       QM9Dataset(stage='val', root=dataset_dir, remove_h=cfg.dataset.remove_h),
                                       QM9Dataset(stage='test', root=dataset_dir, remove_h=cfg.dataset.remove_h)])
        logging.info('QM9_bond: min = ' + str(torch.min(dataset.data.edge_attr)) + ' , max = ' + str(torch.max(dataset.data.edge_attr)))
        pre_transform_in_memory(dataset,
                                partial(pretransform_QM9_generation),
                                show_progress=True)
    return dataset
...
def preformat_AQSOL(dataset_dir):
    """Load and preformat AQSOL loader.

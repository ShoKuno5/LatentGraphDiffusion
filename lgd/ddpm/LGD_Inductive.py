...
from lgd.model.utils import make_beta_schedule, extract_into_tensor, extract_into_sparse_tensor, noise_like, num2batch, \
    exists, default, ismap, mean_flat, count_params, instantiate_from_config
...
class DDPM(OptionalTrainerLightningModule):
    ...
    def __init__(self,
                 gt_config=None,
                 timesteps=100,  # inductive tasks are easier
    ...
    def get_input(self, batch):
        num_node = batch.get('num_node_per_graph', torch.tensor([batch.num_nodes], dtype=torch.long, device=batch.x.device))
        if cfg.dt.get('latent_edge', False):
            batch_idx = torch.cat([batch.batch, num2batch(num_node ** 2)], dim=0)  # TODO: this may not suitable for unpadded data
        else:
...
class LatentDiffusionInductive(DDPM):
    """main class"""
    def __init__(self,
        ...
        if self.cond_stage_key in ['masked_graph', 'prompt_graph']:
        ...
        self.latent_node = cfg.encoder.get("latent_node", True) and cfg.dt.get("latent_node", True)
        self.latent_edge = cfg.encoder.get("latent_edge", False) and cfg.dt.get("latent_edge", False)
        self.latent_graph = cfg.encoder.get("latent_graph", False) and cfg.dt.get("latent_graph", False)
    ...
    def get_input(self, batch, prompt_graph_batch=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        batch.edge_attr_0 = batch.edge_attr.clone().detach() if batch.get('edge_attr', None) is not None else None

        input_label = batch.y.clone().detach() if cfg.train.pretrain.input_target else None
        if input_label is not None and (cfg.dataset.format in ['PyG-Planetoid', 'PyG-Amazon', 'PyG-Coauthor'] or cfg.dataset.transductive):
            input_label = batch.y.clone().detach()[batch.get(batch.split+'_mask', None)].flatten()
        ...
        if not self.latent_node:
            setattr(batch_z, 'x', None)
            # batch_z.x = None
        if not self.latent_edge:
            setattr(batch_z, 'edge_attr', None)
            # batch_z.edge_attr = None
        if not self.latent_graph:
            setattr(batch_z, 'graph_attr', None)
            # batch_z.graph_attr = None
        if not self.first_stage_trainable:
            batch_z.x = batch_z.x.detach() if batch_z.x is not None else None
            batch_z.edge_attr = batch_z.edge_attr.detach() if batch_z.get('edge_attr', None) is not None else None
            batch_z.graph_attr = batch_z.graph_attr.detach() if batch_z.get('graph_attr', None) is not None else None
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key == 'masked_graph':
                ...
                batch_masked.edge_attr = batch.get('edge_attr_masked', None)  #.clone().detach()
                if batch.get("prefix", None) is not None:
                    batch_masked.prefix = batch.prefix
                batch_masked = self.get_learned_conditioning(batch_masked)
                if not self.latent_node:
                    setattr(batch_masked, 'x', None)
                    # batch_masked.x = None
                if not self.latent_edge:
                    setattr(batch_masked, 'edge_attr', None)
                    # batch_masked.edge_attr = None
                if not self.latent_graph:
                    setattr(batch_masked, 'graph_attr', None)
                    # batch_masked.graph_attr = None
                if not self.first_stage_trainable or not self.cond_stage_trainable:
                    batch_masked.x = batch_masked.x.detach() if batch_masked.x is not None else None
                    batch_masked.edge_attr = batch_masked.edge_attr.detach() if batch_masked.get('edge_attr', None) is not None else None
                    batch_masked.graph_attr = batch_masked.graph_attr.detach() if batch_masked.get('graph_attr', None) is not None else None
                xc = (batch.x_masked, batch.get('edge_attr_masked', None))
                c = (batch_masked.x, batch_masked.edge_attr, batch_masked.get('graph_attr', None))
        ...
        batch.x_start = batch_z.x
        batch.edge_start = batch_z.edge_attr
        batch.graph_start = batch_z.get('graph_attr', None)  #.clone().detach()
        batch.c = c
    ...
    @torch.no_grad()
    def decode_first_stage(self, batch_z, **kwargs):
        return self.first_stage_model.decode(batch_z, **kwargs)  # TODO: implement .decode() method for first_stage_model
    ...
    def training_step(self, batch, prompt_graph_batch=None, task_id=None):
        ...
        loss, loss_dict, loss_task, graph_pred, loss_node, loss_edge, loss_graph = self.shared_step(batch, t=t, prompt_graph_batch=prompt_graph_batch, task_id=task_id)
        ...
        return loss, loss_task, graph_pred, loss_node, loss_edge, loss_graph
    ...
    def p_losses(self, batch, cond, t, batch_idx, noise=None):
        ...
        # if not self.latent_node or self.latent_edge or self.latent_graph:
        ...
        batch_noisy.x = x_noisy
        if self.latent_edge:
            edge_start = batch.edge_start.clone().detach()
            noise_edge = default(noise, lambda: torch.randn_like(batch.edge_start))
            batch_noisy.edge_attr = self.q_sample(x_start=batch.edge_start, t=t, noise=noise_edge, batch_idx=batch_idx[batch.num_nodes:])
            if self.parameterization == "x0":
                target_edge = edge_start
            elif self.parameterization == "eps":
                target_edge = noise_edge
            else:
                raise NotImplementedError()
        else:
            batch_noisy.edge_attr = torch.zeros([batch_noisy.edge_index.shape[1], self.hid_dim], dtype=torch.float, device=batch_noisy.x.device)
        ...
        # encoder_pred = self.decode_first_stage(batch)
        # if cfg.dataset.format == 'PyG-QM9':
        ...
        loss_simple = self.get_loss(batch_output.x, target, mean=False).mean([1])
        if self.latent_edge:
            loss_simple_edge = self.get_loss(batch_output.edge_attr, target_edge, mean=False).mean([1])
            loss_simple = torch.cat([loss_simple, loss_simple_edge], dim=0)
        loss_graph = self.get_loss(batch_output.graph_attr, graph_attr_label, mean=False).mean([1]) if self.latent_graph else 0
        ...
        loss_edge = loss[batch.num_nodes:].mean() * self.edge_factor if self.latent_edge else 0
        loss_graph = loss_graph.mean() * self.graph_factor if self.latent_graph else 0
        loss = loss_node + loss_edge + loss_graph

        loss_vlb = self.get_loss(batch_output.x, target, mean=False).mean(dim=1)
        if self.latent_edge:
            loss_vlb_edge = self.get_loss(batch_output.edge_attr, target_edge, mean=False).mean(dim=1)
            loss_vlb = torch.cat([loss_vlb, loss_vlb_edge], dim=0)
        loss_vlb = self.lvlb_weights[t][batch_idx] * loss_vlb
        loss_vlb = loss_vlb[:batch.num_nodes].mean() * self.node_factor
        loss_vlb_edge = loss_vlb[batch.num_nodes:].mean() * self.edge_factor if self.latent_edge else 0
        loss_vlb_graph = (self.get_loss(batch_output.graph_attr, graph_attr_label, mean=False).mean(dim=1) * self.lvlb_weights[t]).mean() * self.graph_factor if self.latent_graph else 0
        loss_vlb = loss_vlb + loss_vlb_edge + loss_vlb_graph
        loss_dict.update({f'{prefix}/loss_vlb': loss_vlb})
        loss += (self.original_elbo_weight * loss_vlb)
        loss_dict.update({f'{prefix}/loss': loss})

        pred = self.decode_first_stage(batch_output, task=cfg.dataset.task, split=batch.split)
        ...
        if cfg.dataset.format in ['PyG-Planetoid', 'PyG-Amazon', 'PyG-Coauthor'] or cfg.dataset.transductive:
            label = batch.y.clone().detach()[batch.get(batch.split + '_mask', None)].flatten()
        else:
            label = batch.y.clone()
        loss_task, graph_decode = compute_loss(pred, label)
        loss_dict.update({f'{prefix}/loss_task': loss_task})

        return loss, loss_dict, loss_task, graph_decode, loss_node, loss_edge, loss_graph
    ...
    def p_mean_variance(self, batch_x, c, t, batch_idx, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        if not self.latent_edge:
            batch_model_out.edge_attr = torch.zeros([batch_model_out.edge_index.shape[1], self.hid_dim], device=batch_model_out.edge_attr.device, dtype=torch.float)
        ...
        if self.parameterization == "eps":
            if self.latent_edge:
                ...
                x_recon = self.predict_start_from_noise(x, t=t, noise=batch_model_out.x, batch_idx=batch_idx)
        elif self.parameterization == "x0":
            if self.latent_edge:
                x_recon = torch.cat([batch_model_out.x, batch_model_out.edge_attr], dim=0)
            else:
                x_recon = batch_model_out.x
        else:
            raise NotImplementedError()
    ...
    def p_sample(self, batch_x, c, t, batch_idx, clip_denoised=False, repeat_noise=False,
        ...
        batch_x.edge_attr = p_sampled_x[batch_x.num_nodes:] if self.latent_edge else \
            torch.zeros([batch_x.edge_index.shape[1], self.hid_dim], device=batch_x.edge_attr.device, dtype=torch.float)
    ...
    def p_sample_loop(self, batch, cond, shape, batch_idx, return_intermediates=False,
        ...
        batch.x = img[: batch.num_nodes]
        batch.edge_attr = img[batch.num_nodes:] if self.latent_edge \
            else torch.zeros([batch.edge_index.shape[1], self.hid_dim], dtype=torch.float, device=device)
        ...
        for i in iterator:
            ...
            if not self.latent_edge:
                batch.edge_attr = torch.zeros([batch.edge_index.shape[1], self.hid_dim], dtype=torch.float, device=device)
            img = torch.cat([batch.x, batch.edge_attr], dim=0)
            if mask is not None:
                img_orig = self.q_sample(x0, ts, batch_idx=batch_idx)
                img = img_orig * mask + (1. - mask) * img
    ...
    def inference(self, batch, sample=True, ddim_steps=None, ddim_eta=0., use_ddpm_steps=False, return_keys=None,
        ...
        pred = self.decode_first_stage(samples, task=cfg.dataset.task, split=batch.split)
        # if cfg.dataset.format == 'PyG-QM9':
        #     pred = pred * batch.get('y_std', 1.) + batch.get('y_mean', 0.)
        if cfg.dataset.format in ['PyG-Planetoid', 'PyG-Amazon', 'PyG-Coauthor'] or cfg.dataset.transductive:
            label = batch.y.clone().detach()[batch.get(batch.split + '_mask', None)].flatten()
        else:
            label = batch.y.clone()
        loss, pred = compute_loss(pred, label)
        ...
        #             z_noisy = self.q_sample(x_start=z_start, t=t, noise=noise, batch_idx=batch_idx)  # TODO: should align with z_start
        ...
        #     # get denoise row
        #     with self.ema_scope("Plotting"):
        #         samples, z_denoise_row = self.sample_log(cond=c, batch_size=N, batch_idx=batch_idx, ddim=use_ddim,
        ...
        #
        #             samples, _ = self.sample_log(cond=c, batch_size=N, batch_idx=batch_idx, ddim=use_ddim, eta=ddim_eta,
        ...
        #                                                        shape=(batch_idx.shape[0], self.hid_dim),  # TODO: change progressive_denoising
        #                                                        batch_idx=batch_idx,
        #                                                        batch_size=N)
...
class DiffusionWrapper(OptionalTrainerLightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self.diffusion_model = eval(cfg.dt.get('type', 'DenoisingTransformer'))(cfg)
...
class Layout2ImgDiffusion(LatentDiffusionInductive):

...
#PJM -j
#PJM -N lgd_zinc_uncond
#PJM -o lgd_%j.out
#PJM -e lgd_%j.err

# =============================================================
# ZINC unconditional diffusion launcher
# =============================================================
...
EXP="${EXP_PREFIX}zinc_uncond_${TS}"
EXP_DIR="$RUNS/$EXP"
...
if [ -n "$USER_WANDB_PROJECT" ]; then
  ...
  WANDB_PROJECT="LGD-ZINC-Unconditional"
fi
...
if [[ -z "${WANDB_NAME}" ]]; then
  WANDB_NAME="zinc_uncond_${EXP}"
fi
export WANDB_NAME

echo "=== ZINC Unconditional Diffusion Job ==="
echo "Config      : ${CONFIG}"
...
{
  echo "ZINC unconditional diffusion job completed at $(date)"
  echo "Config=${CONFIG}"
  echo "Checkpoint=${CHECKPOINT}"
  echo "Repeat=${REPEAT}"

...
# =============================================================
# ZINC diffusion (conditional) launcher
# =============================================================
...
EXP="${EXP_PREFIX}zinc_diffusion_${TS}"
EXP_DIR="$RUNS/$EXP"
...
if [ -n "$USER_WANDB_PROJECT" ]; then
  ...
  WANDB_PROJECT="LGD-ZINC-Diffusion"
fi
...
if [[ -z "${WANDB_NAME}" ]]; then
  WANDB_NAME="zinc_diffusion_${EXP}"
fi
export WANDB_NAME

echo "=== ZINC Diffusion Job ==="
echo "Config      : ${CONFIG}"
...
{
  echo "ZINC diffusion job completed at $(date)"
  echo "Config=${CONFIG}"
  echo "Checkpoint=${CHECKPOINT}"
  echo "Repeat=${REPEAT}"

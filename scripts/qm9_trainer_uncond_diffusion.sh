...
# =============================================================
# QM9 unconditional diffusion (simple/complex) launcher
# Override via: pjsub -x CONFIG=cfg/QM9_unconditional_generation_diffusion_simple.yaml ...
# =============================================================
CONFIG=${CONFIG:-cfg/QM9_unconditional_generation_diffusion_simple.yaml}
...
EXP="${EXP_PREFIX}qm9_uncond_diff_${TS}"
EXP_DIR="$RUNS/$EXP"
...
if [ -n "$USER_WANDB_PROJECT" ]; then
  ...
  WANDB_PROJECT="LGD-QM9-Uncond-Diffusion"
fi
...
if [[ "$CHECKPOINT" == "auto" ]]; then
  ...
  mapfile -t CANDIDATES < <(find /workspace -type f \
    \( -path "*/QM9_unconditional_generation_encoder/*/ckpt/*.ckpt" \
       -o -path "*/qm9_unconditional_generation_encoder/*/ckpt/*.ckpt" \) 2>/dev/null | sort -V)
  if (( ${#CANDIDATES[@]} == 0 )); then
    echo "ERROR: No QM9 unconditional encoder checkpoint found. Set CHECKPOINT explicitly."
    exit 1
  fi
...
if [[ -z "${WANDB_NAME}" ]]; then
  WANDB_NAME="qm9_uncond_diff_${EXP}"
fi
export WANDB_NAME

echo "=== QM9 Unconditional Diffusion ==="
echo "Config      : ${CONFIG}"
...
{
  echo "QM9 diffusion job completed at $(date)"
  echo "Config=${CONFIG}"
  echo "Checkpoint=${CHECKPOINT}"
  echo "Seed=${SEED}"
  echo "Repeat=${REPEAT}"

...
if __name__ == '__main__':
    ...
    for run_id, seed, split_index in zip(*run_loop_settings()):
        ...
        model = LatentDiffusion(conditioning_key=cfg.diffusion.conditioning_key, hid_dim=cfg.diffusion.hid_dim,
                                cond_stage_key=cfg.diffusion.cond_stage_key, first_stage_config=cfg.diffusion.first_stage_config,
                                cond_stage_config=cfg.diffusion.cond_stage_config, edge_factor=cfg.diffusion.edge_factor).to(torch.device(cfg.accelerator))
        ...
        w = GraphTransformerEncoder(cfg=cfg.regression_model.encoder).to(torch.device(cfg.accelerator))
        w = init_model_from_pretrained(
            w, cfg.regression_model.pretrained.dir, cfg.regression_model.pretrained.freeze_main,
            cfg.regression_model.pretrained.reset_prediction_head
        )
        ...
        if cfg.train.mode == 'standard':
            ...
            train_dict[cfg.train.mode](loggers, loaders, model, w, optimizer,
                                       scheduler)

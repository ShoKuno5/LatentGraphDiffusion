#!/bin/bash
# Azure VM wrapper for ZINC diffusion training (masked_graph)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
source "$REPO_ROOT/scripts/env/azure.sh"
...
echo "[azure] Starting diffusion training: $EXP"
...
if command -v singularity &>/dev/null; then
  ...
  bash scripts/common/run_zinc_train_diffusion.sh \
    --checkpoint "$ENCODER_CHECKPOINT" \
    --config "$CONFIG" \
    --repeat "$REPEAT" \
    --max-epoch "$MAX_EPOCH" \
